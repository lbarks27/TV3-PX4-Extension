...
The patch is applied during `scripts/prepare_px4_tree.sh`, which then performs additional integration steps (module.yaml edits, allocator registration, SIH startup overlays, commander command passthrough, build flag adjustments, etc.) so the full rocket feature set works against the selected PX4 tag while keeping the majority of the behavior out-of-tree in this repo.
