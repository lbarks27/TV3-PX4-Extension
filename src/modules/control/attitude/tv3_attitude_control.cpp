...
class TV3AttitudeControl : public ModuleBase<TV3AttitudeControl>, public ModuleParams, public px4::ScheduledWorkItem
{
public:
	TV3AttitudeControl() :
	...
	{
		TV3AttitudeControl *instance = new TV3AttitudeControl();
	...
	static int print_usage(const char *reason = nullptr)
	{
		...
		PRINT_MODULE_DESCRIPTION("TV3 attitude/rate controller publishing body torque setpoints.");
		PRINT_MODULE_USAGE_NAME("tv3_attitude_control", "modules");
...
private:
	void Run() override
	{
		...
		if (_status.mode < tv3_status_s::MODE_READY || _status.mode == tv3_status_s::MODE_ABORT
		    || coast_without_guidance || !_attitude_setpoint.q_d_valid) {
			reset_controller();
		...
		Quatf q(_attitude.q);
		Quatf q_ref(_attitude_setpoint.q_d);
...
extern "C" __EXPORT int tv3_attitude_control_main(int argc, char *argv[])
{
	return TV3AttitudeControl::main(argc, argv);
}
