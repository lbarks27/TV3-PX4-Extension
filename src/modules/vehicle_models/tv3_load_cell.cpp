...
private:
	...
	void Run() override
	{
		...
		const bool sensor_valid = (fault_flags & (tv3_thrust_s::FAULT_STALE | tv3_thrust_s::FAULT_CHANNEL_MISSING
							 | tv3_thrust_s::FAULT_BAD_SCALE)) == 0;
	...
	void update_engine_state_from_reference(uint8_t aggregate_fault_flags)
	{
		...
		for (int i = 0; i < engine_count; ++i) {
			...
			_engine_filtered_thrust_n[i] = kSlowAlpha * _engine_measured_thrust_n[i] + (1.f - kSlowAlpha) * _engine_filtered_thrust_n[i];
