...
class VehicleAssetTests(unittest.TestCase):
    def test_generated_tv3_params_match_firmware_definitions(self) -> None:
        ...
        defined_params = set(
            re.findall(r"PARAM_DEFINE_(?:INT32|FLOAT)\((RK_[A-Z0-9_]+),", Path("src/modules/tv3_params/tv3_params.c").read_text())
        )
    ...
    def test_generate_vehicle_assets(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("tv3_attitude start", runtime_extras)
            self.assertNotIn("tv3_control_mixer start", runtime_extras)
            self.assertNotIn("tv3_guidance start", runtime_extras)
            self.assertIn("vehicle_attitude 20", logger_topics)
            self.assertIn("vehicle_attitude_groundtruth 20", logger_topics)
            self.assertIn("vehicle_angular_velocity 20", logger_topics)
            self.assertIn("vehicle_angular_velocity_groundtruth 20", logger_topics)
            self.assertIn("vehicle_local_position_groundtruth 50", logger_topics)
            self.assertIn("vehicle_global_position 50", logger_topics)
            self.assertIn("vehicle_global_position_groundtruth 50", logger_topics)
            self.assertIn("vehicle_torque_setpoint 20", logger_topics)
            self.assertIn("tv3_plant_wrench 20", logger_topics)
            self.assertIn("vehicle_command_ack 50", logger_topics)
            self.assertIn("tv3_status 20", logger_topics)
            self.assertIn("tv3_gimbal_command 20", logger_topics)
    ...
    def test_flight_profile_overlay_generates_guidance_params(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual("3.0", params["RK_GD_TAKE_ALT"])
            self.assertEqual("8.0", params["RK_GD_APEX_ALT"])
            self.assertEqual("3.0", params["RK_GD_ACC_RAD"])
            self.assertEqual("0.0", params["RK_GD_WP1_N"])
            self.assertEqual("-3.0", params["RK_GD_WP1_D"])
            ...
            self.assertAlmostEqual(6.0, float(params["RK_SIM_BURN_SCL"]), places=3)
            self.assertEqual("1", params["RK_GD_BOOST_FULL"])
            self.assertAlmostEqual(7.5, float(params["RK_ATT_P_BOOST"]), places=3)
            self.assertAlmostEqual(2.5, float(params["RK_ATT_VEL_P"]), places=3)
            self.assertAlmostEqual(1.8, float(params["RK_RATE_P_BOOST"]), places=3)
            self.assertAlmostEqual(0.8, float(params["RK_SIH_RAIL_RAMP"]), places=3)
            self.assertAlmostEqual(5.0, float(params["RK_SIH_RATE_DAMP"]), places=3)
            self.assertAlmostEqual(0.52, float(params["RK_GD_POS_P"]), places=3)
            self.assertAlmostEqual(6.72, float(params["RK_BODY_MASS_KG"]), places=3)
            self.assertAlmostEqual(2.0, float(params["RK_IXX"]), places=3)
            self.assertAlmostEqual(2.0, float(params["RK_IYY"]), places=3)
            self.assertAlmostEqual(0.25, float(params["RK_IZZ"]), places=3)
            self.assertTrue(
                (output / "runtime" / "fs" / "microsd" / "tv3" / "motors" / "aerotech-g12" / "curve.csv").exists()
            )
    ...
    def test_sitl_airframes_share_common_defaults(self) -> None:
        ...
        sih_rc = Path("overlay/ROMFS/init.d-posix/px4-rc.sihsim").read_text()
        ...
        self.assertIn("tv3_sih_ideal start", sih_rc)
        self.assertIn("TV3_SIH_IDEAL", sih_rc)
        self.assertTrue(Path("src/modules/simulation/tv3_sih_ideal/tv3_sih_ideal.cpp").exists())

        post = Path("overlay/ROMFS/init.d-posix/airframes/tv3_common.post").read_text()
        self.assertIn("tv3_mode_manager start", post)
        self.assertIn("tv3_attitude start", post)
        self.assertIn("tv3_control_mixer start", post)
